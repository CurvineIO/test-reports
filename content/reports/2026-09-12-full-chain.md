...
| perf-benchmark | NOT_RECORDED | PASS | 5m 29s | passed | PASSED |
...
| Sequential read 256KB | 2.77 | 11328.61 | 20.88 ms/op | 22.94 | 26.35 | 27.92 | 45.38 | 65536 | 0 | 3.11 | -11.1% | 26.08 | +7.0% | degraded |
...
- Root cause: Rust compiler query recursion limit is exceeded while type-checking async test code in fallback_read_test and write_cache_test crates. Whether increased type complexity or toolchain limits is the primary cause remains unproven.
- Recommendation: Bisect commits between the last passing daily SHA and the current main head to identify async test changes that increased type depth.
...
- Root cause: curvine-fuse buffered read/write on the FUSE mount returns Input/output error during sustained or verified writes. This matches the pre-existing defect tracked in GitHub issue #1646.
- Recommendation: Review FuseConf-to-mount flag mapping in curvine-fuse mount CLI code and restore correct buffered I/O behavior per open issue #1646.
...
- Recommendation: Reuse existing repair track for GitHub issue #1646.

## Follow-up

### Defects and fixes

- GitHub Issue: #1646 (reused)
...
- Bisect async test compilation failures blocking daily and integration profiles.
- Continue repair for GitHub issue #1646 buffered FUSE EIO defect.
